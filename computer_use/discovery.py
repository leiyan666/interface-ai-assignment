    ...
    try:
        ...
        for step_number in range(1, max_steps + 1):
            ...
            if action.action_type == "escalate":
                handoff(surface, logger, "evidence/failure_or_handoff.png")
                continue
            validate_action(action, compile_lookup_capability(target, []).safety_policy, surface.current_url())
            result = surface.execute(action)
            trajectory.append({"step": step_number, "action": action.model_dump(), "result": result.model_dump()})
            logger.event("action_executed", step=step_number, action=action.action_type, reason=action.reason, status="ok")
